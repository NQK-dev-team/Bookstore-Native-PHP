...
try {
      ...
      $stmt2 = $conn->prepare("update appUser join customer on appUser.id = customer.id set email=null,phone=null,deleteTime=null,referrer=null,imagePath=null,address=null,password=null,name='ACCOUNT REMOVED' where customer.id=?");
      if (!$stmt2) {
            $logMessage = $currentDateTime . " - MySQL Query `update appUser join customer on appUser.id = customer.id set email=null,phone=null,deleteTime=null,referrer=null,imagePath=null,address=null,password=null,name='ACCOUNT REMOVED' where customer.id=?` Preparation Failed!\n";
            file_put_contents(__DIR__ . '/delete_account.log', $logMessage, FILE_APPEND);
            exit;
      }
