...
) as combined join book on book.id=combined.bookID and book.status=true group by bookID order by combined.totalSold desc,name limit 5');
            if (!$stmt) {
                  http_response_code(500);
...
union
select bookID,count(*) as totalSold from fileOrderContain join customerOrder on customerOrder.id=fileOrderContain.orderID where customerOrder.status=true and week(purchaseTime,1)=week(curdate(),1) group by bookID
) as combined join book on book.id=combined.bookID and book.status=true group by bookID order by combined.totalSold desc,name limit 5` preparation failed!']);
                  $conn->close();
                  exit;
            }
            if (!$stmt->execute()) {
