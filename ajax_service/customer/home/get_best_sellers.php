...
) as combined join book on book.id=combined.bookID and book.status=true group by bookID,name,edition,isbn,publisher,publishDate,imagePath,avgRating,description order by combined.totalSold desc,name limit 5');
            if (!$stmt) {
                  http_response_code(500);
