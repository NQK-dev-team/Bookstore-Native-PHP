...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
      if (isset($_GET['code'])) {
            try {
                  ...
                  $stmt = $conn->prepare('select imagePath,name,edition,isbn,ageRestriction,publisher,publishDate,description,avgRating,amount,book.id,physicalCopy.price
                  ...
                  if (!$stmt) {
                        http_response_code(500);
                        echo json_encode(['error' => 'Query `select imagePath,name,edition,isbn,ageRestriction,publisher,publishDate,description,avgRating,amount,book.id,physicalCopy.price
                  from book join physicalOrderContain on physicalOrderContain.bookID=book.id
                  join physicalCopy on physicalCopy.id=book.id
                  join customerOrder on customerOrder.id=physicalOrderContain.orderID
