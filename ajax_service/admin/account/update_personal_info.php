...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      if (
            ...
            try {
                  ...
                  $stmt = $conn->prepare("UPDATE appUser JOIN admin ON appUser.id = admin.id SET name=?, phone=?, address=?, dob=?, gender=? WHERE admin.id=?");
                  if (!$stmt) {
                        http_response_code(500);
                        echo json_encode(['error' => 'Query `UPDATE appUser JOIN admin ON appUser.id = admin.id SET name=?, phone=?, address=?, dob=?, gender=? WHERE admin.id=?` preparation failed!']);
                        $conn->rollback();
                        $conn->close();
                        exit;
                  }
                  ...
                  if (isset($_FILES['image'])) {
                        ...
                        $stmt = $conn->prepare('select imagePath from appUser join admin on admin.id=appUser.id where admin.id=?');
                        if (!$stmt) {
                              http_response_code(500);
                              echo json_encode(['error' => 'Query `select imagePath from appUser join admin on admin.id=appUser.id where admin.id=?` preparation failed!']);
                              $conn->rollback();
                              $conn->close();
                              exit;
                        }
                        ...
                        if (!$isSuccess) {
                              ...
                              if ($result['imagePath']) {
                                    $path = dirname(dirname(dirname(__DIR__))) . "/data/user/admin/" . $result['imagePath'];
                              ...
                              if (!is_dir(dirname(dirname(dirname(__DIR__))) . "/data/user/admin/" . $imageDir)) {
                                    if (!mkdir(dirname(dirname(dirname(__DIR__))) . "/data/user/admin/" . $imageDir)) {
                                          $conn->rollback();
                                          $conn->close();
                              ...
                              if (!move_uploaded_file($_FILES["image"]["tmp_name"], dirname(dirname(dirname(__DIR__))) . "/data/user/admin/" . $imageFile)) {
                                    $conn->rollback();
                                    $conn->close();
                                    throw new Exception("Error occurred during moving image file!");
                              }

                              $stmt = $conn->prepare('update appUser join admin on admin.id=appUser.id set imagePath=? where admin.id=?');
                              if (!$stmt) {
                                    http_response_code(500);
                                    echo json_encode(['error' => 'Query `update appUser join admin on admin.id=appUser.id set imagePath=? where admin.id=?` preparation failed!']);
                                    $conn->close();
                                    exit;
                              }
                              ...
                              if (!$isSuccess) {
                                    ...
                                    if ($stmt->affected_rows > 1) {
                                          http_response_code(500);
                                          echo json_encode(['error' => 'Updated more than one admin account!']);
                                          $stmt->close();
                                          $conn->rollback();
                                          $conn->close();
                                          exit;
                  ...
                  $stmt = $conn->prepare('select email from appUser join admin on admin.id=appUser.id where admin.id=?');
                  if (!$stmt) {
                        http_response_code(500);
                        echo json_encode(['error' => 'Query `select email from appUser join admin on admin.id=appUser.id where admin.id=?` preparation failed!']);
                        exit;
                  }
                  $stmt->bind_param('s', $_SESSION['id']);
                  $isSuccess = $stmt->execute();
                  ...
                  $stmt->close();
                  $conn->close();
                  personal_info_change($email, 'admin');
